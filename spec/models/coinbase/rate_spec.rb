...
RSpec.describe Coinbase::Rate, type: :model do
  ...
  let!(:btc_rate) { Coinbase::Rate.create pair: btc_usd, low: 31_885.88, high: 31_919.65, open: 31_911.23, close: 31_910.06, volume: 6.24052656, interval: 60, time: Time.now - 1.day }
  let!(:eth_rate) { Coinbase::Rate.create pair: eth_usd, low: 1435.25, high: 1440.0, open: 1435.67, close: 1438.5, volume: 596.98884737, interval: 60, time: Time.now }
  ...
  describe '.paginated' do
    let(:result) { Coinbase::Rate.paginated(Base64.encode64("before__#{eth_rate.time.to_i}")) }
